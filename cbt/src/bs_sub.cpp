...
#include "cbt/Boiler.h" // MsgTutorial 메시지 파일 헤더 (빌드 후 자동 생성됨)
...
void msgCallback(const cbt::Boiler::ConstPtr& msg)
{
ROS_INFO("state = %d", msg->state);
}
int main(int argc, char **argv) // 노드 메인 함수
{
ros::init(argc, argv, "boiler_sub"); // 노드명 초기화
...
ros::Subscriber ros_tutorial_sub = nh.subscribe("boiler_msg", 100, msgCallback);
